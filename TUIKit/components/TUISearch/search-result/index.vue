<template>
	...
	<div v-else :class="['tui-search-result', !isPC && 'tui-search-result-h5', isPC && isResultDetailShow && 'tui-search-result-with-border']">
		<div v-if="props.searchType !== 'conversation' && (isPC || !isResultDetailShow)" class="tui-search-result-main">
			<div class="tui-search-result-list">
				<div v-for="result in searchResult" :key="result.key" class="tui-search-result-list-item">
					...
					<div class="list">
						<div v-for="(item, index) in result.list" :key="`${result.key}-${index}`" :class="[generateListItemClass(item)]">
							<SearchResultItem
					...
					<div v-if="currentSearchTabKey === 'all' || result.cursor" class="more" @click="getMoreResult(result)">
		...
		<div
			...
			:class="['tui-search-result-detail', props.searchType === 'conversation' && 'tui-search-result-in-conversation']"
		>
			<SearchResultLoading v-if="isSearchDetailLoading" :class="['search-result-loading', !isPC && 'search-result-loading-h5']" />
			<div v-if="!isSearchDetailLoading && isResultDetailShow && props.searchType !== 'conversation'" class="tui-search-message-header">
				<div class="header-content">
					...
					<div class="header-content-type normal">
						<span>{{ TUITranslateService.t('TUISearch.相关的') }}</span>
						<span>{{ TUITranslateService.t(`TUISearch.${currentSearchTabKey === 'allMessage' ? '结果' : searchMessageTypeList[currentSearchTabKey].label}`) }}</span>
					</div>
				</div>
			...
			<div v-if="!isSearchDetailLoading && searchConversationMessageList && searchConversationMessageList[0]" class="tui-search-message-list">
				<template v-if="props.searchType === 'global' || (currentSearchTabKey !== 'imageMessage' && currentSearchTabKey !== 'fileMessage')">
					<div v-for="item in searchConversationMessageList" :key="generateVueRenderKey(item.ID)" :class="['list-item']">
						<SearchResultItem
							:listItem="item"
				...
				<template v-else>
					...
					>
						...
						<div v-for="item in group.list" :key="generateVueRenderKey(item.ID)" :class="['list-group-item']">
							<SearchResultItem
								:listItem="item"
								:listItemContent="item.getMessageContent()"
								:type="currentSearchTabKey"
				...
				<div v-if="searchConversationResult && searchConversationResult.cursor" class="more" @click="getMoreResultInConversation">
					<Icon class="more-icon" :file="searchIcon" width="12px" height="12px" />
					<div class="more-text">
...
const props = defineProps({
	...
	autoShowSearch: { type: Boolean, default: true } // 新增
});

// search params
const keywordList = ref<string[]>([]);
const messageTypeList = ref<string | string[]>(searchMessageTypeDefault[props.searchType as SEARCH_TYPE]?.value as string[]);
...
const searchConversationResultGroupByDate = ref<Array<{ date: string; list: IMessageModel[] }>>([]);
...
const setMessageSearchResultList = (option?: { conversationID?: string | undefined; cursor?: string | undefined }) => {
	...
	} as SearchCloudMessagesParams).then((res: { data: ISearchCloudMessageResult }) => {
		enableSampleTaskStatus('searchCloudMessage');
		if (!option?.conversationID) {
			option?.cursor
				? (searchAllMessageList.value = [...searchAllMessageList.value, ...res.data.searchResultList])
			...
			option?.cursor
				? (searchConversationMessageList.value = [...searchConversationMessageList.value, ...(res?.data?.searchResultList[0]?.messageList as IMessageModel[])])
...
watch(
	...
	(newValue, oldValue) => {
		if (!props.autoShowSearch) return;
...
const groupResultListByDate = (messageList: IMessageModel[]): Array<{ date: string; list: IMessageModel[] }> => {
